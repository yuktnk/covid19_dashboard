...
export const fetchAsyncGetDaily = createAsyncThunk(`${name}/getDaily`, async () => {
    const { data } = await axios.get<API_DATA_DAILY>(`${apiUrl}/daily`);
    return data;
});

export const fetchAsyncGetCountry = createAsyncThunk(`${name}/getCountry`, async (country: string) => {
...
export const seelectCountry = (state: RootState) => state.covid.country
